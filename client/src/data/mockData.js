// Mock data for the Mini-ERP system
...
export function getQuoteById(id) {
  ...
  if (quote) {
    quote.customer = getCustomerById(quote.customerId);
...
export function getSalesOrderById(id) {
  ...
  if (order) {
    ...
    order.quote = getQuoteById(order.quoteId);
  }
  return order;
}
...
export function getShipmentById(id) {
  ...
  if (shipment) {
    shipment.customer = getCustomerById(shipment.customerId);
    shipment.salesOrder = getSalesOrderById(shipment.salesOrderId);
  }
  return shipment;
}
...
export function getAllShipmentsWithDetails() {
  return mockShipments.map(s => {
    const shipment = { ...s };
    shipment.customer = getCustomerById(shipment.customerId);
    shipment.salesOrder = getSalesOrderById(shipment.salesOrderId);
    return shipment;
